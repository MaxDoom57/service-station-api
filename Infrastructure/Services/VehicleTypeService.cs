    ...
    public class VehicleTypeService
    {
        ...
        public async Task<List<VehicleTypeDto>> GetVehicleTypesAsync()
        {
            ...
            return await db.CdMas
                .Where(x => x.CKy == _userContext.CompanyKey && x.ConCd == ConCd && !x.fInAct)
        ...
        public async Task<(bool success, string message)> AddVehicleTypeAsync(CreateVehicleTypeDto dto)
        {
            ...
            // Look up ConKy for OrdCat1
            var existingType = await db.CdMas.FirstOrDefaultAsync(x => x.ConCd == ConCd && x.CKy == _userContext.CompanyKey);
            short conKy = existingType?.ConKy ?? 1; // Fallback
